...
- Polaris o Nessie?
- Minio
- Orchestratore (Workflow Manager, Kubeflow, ?)
- Business Data Catalog
- Dremio (autenticazione) / Trino
- GESTIONE DATI PERSONALI / SENSIBILI
- Ambiente sviluppo (demo)

Monitoring: prometheus/grafana

POLARIS/NESSIE: supportati entrambi

Minio: pipeline build immagine

Argo workflow: gestione automazioni

Lineage:
- No Lineage e2e
- openmetadata

INGESTION: spark / duckdb
...
- [https://trino.io/docs/current/overview.html](https://trino.io/docs/current/overview.html)
- [https://blog.dataengineerthings.org/build-a-lakehouse-on-a-laptop-with-dbt-airflow-trino-iceberg-and-minio-0700540a183d](https://blog.dataengineerthings.org/build-a-lakehouse-on-a-laptop-with-dbt-airflow-trino-iceberg-and-minio-0700540a183d)
- dbt-trino [https://docs.getdbt.com/docs/core/connect-data-platform/trino-setup](https://docs.getdbt.com/docs/core/connect-data-platform/trino-setup)

**DREMIO**

- Query Engine Federato
- Catalogo dati con Layer Semantico
- Accelerazione query (Reflections)
- Interfaccia Evoluta
- Versioni Community / Enterprise / Cloud
- Supporto Iceberg / Nessie

**TRINO**

- Query Engine Federato
- Nessun catalogo dati
- Accelerazione query
- Interfaccia monitoraggio cluster
- Versioni: OS / Cloud
- Supporto Iceberg / Nessie
    - [https://projectnessie.org/iceberg/trino/](https://projectnessie.org/iceberg/trino/)
...
| **Architettura**               | Architettura a due livelli con un "coordinator" per la pianificazione delle query e l'ottimizzazione e degli "executor" per l'esecuzione. | Architettura master-slave con un "coordinator" che analizza, pianifica e schedula le query e dei "worker" che eseguono i task. |
| **Ottimizzazione delle Query** | Utilizza "Data Reflections" (viste materializzate ottimizzate) e "Columnar Cloud Cache" (C3) per accelerare le query.                     | Ottimizzazione basata sul costo e "predicate pushdown" per delegare il filtraggio alle sorgenti dati.                          |
...
| **SQL Dialect**                | SQL-92 con estensioni proprie.                                                                                                            | Conforme allo standard ANSI SQL.                                                                                               |
...
- **Accelerazione delle query:** Le "Data Reflections" possono migliorare drasticamente le performance delle query, specialmente per dashboard e reportistica, pre-calcolando aggregazioni e join.
...
- **Integrazione con Apache Iceberg:** Dremio offre un forte supporto per Apache Iceberg, inclusa l'integrazione con cataloghi come Nessie per la gestione versionata dei dati.
...
Trino eccelle nell'esecuzione di query SQL federate ad alte prestazioni su una vasta gamma di sorgenti dati. La sua architettura MPP (Massively Parallel Processing) lo rende ideale per analisi interattive e ad-hoc su grandi volumi di dati.

**Pro:**

- **Flessibilità e apertura:** Essendo un progetto open source con una forte community, Trino offre un'ampia gamma di connettori e un'elevata interoperabilità.
...
- **Nessun data lock-in:** Trino non memorizza dati, ma si limita a leggerli dalle sorgenti, garantendo la massima libertà nella scelta delle tecnologie di storage.
- **Standard SQL:** L'aderenza allo standard ANSI SQL facilita l'integrazione con strumenti di BI e l'utilizzo da parte di analisti e data scientist.
...
- **Dremio:** Offre connettori nativi e ottimizzati per Oracle, Postgres e SQL Server. L'integrazione con cataloghi Iceberg come Nessie è un punto di forza, permettendo una gestione "git-like" dei dati. Per quanto riguarda MinIO (S3), Dremio può connettersi e sfruttare il suo "Columnar Cloud Cache" per migliorare le performance.
- **Trino:** Dispone di connettori JDBC per i database relazionali, che permettono di interrogare Oracle, Postgres e SQL Server. La configurazione avviene tramite file di properties per ogni catalogo. L'integrazione con Iceberg è solida e supporta diversi cataloghi, inclusi Nessie e Polaris. L'accesso a MinIO (S3) è garantito dal supporto per il file system S3.

### Trasformazioni con dbt

- **Dremio:** L'integrazione con dbt è possibile tramite il connettore `dbt-dremio`. Questo permette di orchestrare le trasformazioni SQL direttamente su Dremio, sfruttando il suo motore di query e il semantic layer per materializzare i modelli dbt come viste o tabelle. Esistono casi di studio che dimostrano l'implementazione di pipeline CI/CD complete con Dremio e dbt.
- **Trino:** Anche Trino si integra con dbt attraverso l'adattatore `dbt-trino`. dbt può inviare le query di trasformazione a Trino, che a sua volta le esegue sulle sorgenti dati federate. Questo approccio permette di centralizzare la logica di trasformazione in dbt e sfruttare la potenza di Trino per l'esecuzione distribuita.
...
- **Trino:** DataHub si integra con Trino per raccogliere metadati sui cataloghi, schemi, tabelle e colonne. Grazie alla capacità di Trino di interrogare diverse sorgenti, DataHub può ottenere una visione federata dei metadati. È possibile tracciare la lineage a livello di tabella e colonna, anche per le viste create in Trino.

### Ambiente Kubernetes

- **Dremio:** Dremio offre un operatore Kubernetes ufficiale che semplifica notevolmente il deployment, la gestione e lo scaling di un cluster Dremio su Kubernetes.
- **Trino:** Trino può essere deployato su Kubernetes utilizzando Helm chart. Sebbene non ci sia un operatore ufficiale mantenuto dal progetto Trino, esistono diverse implementazioni e guide della community per la gestione di Trino su Kubernetes.
